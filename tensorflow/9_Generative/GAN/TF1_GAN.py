...
def nn_G(x):
	...
	OL = tf.nn.sigmoid(tf.add(tf.matmul(HL, W['OL_G']), b['OL_G'])) # binary의 이미가 아니라, G의 output이 되는 mnist의 pixel 값이 0~1사이기 때문에 사용
	return OL

# Neural Network: Discriminator
def nn_D(x):
	HL = tf.nn.relu(tf.add(tf.matmul(x, W['HL_D']), b['HL_D']))
	OL = tf.nn.sigmoid(tf.add(tf.matmul(HL, W['OL_D']), b['OL_D'])) # binary의 의미
	return OL
